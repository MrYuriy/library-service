...
class BorrowingSerializerTest(TestCase):
    def setUp(self):
        # Create test data for BorrowingSerializer
        self.user = User.objects.create_user(username='testuser', email='testuser@example.com', password='testpassword')
        self.book = Book.objects.create(title='Test Book', author='Test Author', cover='HARD', inventory=10, dayly_fee='9.99')
        self.borrowing = Borrowing.objects.create(extend_return_date='2023-07-20', book=self.book, user=self.user)
...
class BorrowingDetailSerializerTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='testuser', email='testuser@example.com', password='testpassword')
        self.book = Book.objects.create(title='Test Book', author='Test Author', cover='HARDCOVER', inventory=10, dayly_fee='9.99')
        self.borrowing = Borrowing.objects.create(extend_return_date='2023-07-20', book=self.book, user=self.user)
